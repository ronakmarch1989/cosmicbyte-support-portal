...
v2.22.0 addition: launches the Discord bot as a subprocess BEFORE the
os.execvp call. The subprocess survives the exec (gets reparented to PID 1)
and keeps running for the lifetime of the Render container. Standard Linux
behaviour. Gated on DISCORD_BOT_TOKEN being set so this file works exactly
as before in environments that don't have a Discord bot configured.
"""
import os
import sys
import subprocess
...
# 5. v2.22.0: launch the Discord bot subprocess. Gated on DISCORD_BOT_TOKEN
# being set, so this file behaves exactly as before in environments without
# the bot configured. The subprocess survives the os.execvp below: when the
# parent process gets replaced by Streamlit, the child gets reparented to
# PID 1 and keeps running. Standard Linux process semantics; works fine on
# Render's single-instance web service.
#
# We don't redirect stdout/stderr so the bot's logs end up in Render's
# combined log stream, same as the portal's logs.
print("=== Discord bot ===")
if os.environ.get("DISCORD_BOT_TOKEN"):
    try:
        ...
        print("✓ Discord bot subprocess launched")
    except Exception as e:
        print(f"  Discord bot launch FAILED: {e} -- continuing without it")
else:
    print("  DISCORD_BOT_TOKEN not set -- skipping Discord bot launch")
